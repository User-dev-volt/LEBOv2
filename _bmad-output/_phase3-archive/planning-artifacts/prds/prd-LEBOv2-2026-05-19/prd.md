...
FR-F4: The passive tree canvas renders a dark stone/obsidian-textured background using a PixiJS `TilingSprite` backed by a bundled 256×256 WebP stone tile texture (`bg_stone_tile.webp`). The TilingSprite is the first child of `worldContainer` (inserted before `edgeGraphics`), making it render beneath all nodes and edges. `background` in `app.init()` changes to transparent (`0x000000`, alpha 0). The texture is loaded once at renderer init and reused across all passive and skill tree canvases. Asset sourcing: attempt extraction from game files via the existing icon pipeline; fall back to a synthetic stone texture generated by a one-time Perlin noise script stored as a static bundled resource.
...
FR-F6: The Weaver Tree tab displays a void/crystalline purple aesthetic using `bg_weaver_tile.webp` as a background — applied to `WeaverTreePlaceholder` as a CSS background image in Phase 3 (since no PixiJS canvas exists yet). When community Weaver Tree node data becomes available, the placeholder is replaced by a full PixiJS canvas with the same texture loaded as a `TilingSprite` (matching FR-F4's approach). Either way, the Weaver tab looks intentional and on-brand rather than a blank panel. The `bg_weaver_tile.webp` asset is sourced via the same pipeline as FR-F4.
...
~~OQ-3: **Idol grid layout data**~~ — **RESOLVED:** The LE idol grid is a 5×5 space (~20 active cells after blocked corners and center). Phase 3 delivers the default grid layout (no altar variants). The grid layout is encoded as a JSON data file bundled with the app (per NFR-6), not hardcoded in UI logic. **Idol Altar support** (13 altar subtypes, each with a unique cell layout and Refracted Slots that amplify affix values) is deferred to Phase 4. The data file structure must support altar variants as future additions without breaking the grid component. The default grid layout spec must be sourced from community documentation (lastepochtools.com/guide/section/idols and wiki) as a one-time data authoring task before Epic C.

~~OQ-4: **Tree background assets**~~ — **RESOLVED:** Backgrounds are implemented as **PixiJS `TilingSprite` with WebP texture assets**. Rationale: `TilingSprite` is zero per-frame overhead (one static draw call), `Sprite` is already imported in `pixiRenderer.ts`, and the existing icon pipeline infrastructure in `icon_commands.rs` can be extended to extract a stone tile texture from game files. Total asset budget < 50KB (two textures: stone base + weaver void). The damage-type tint overlay (FR-F5) is a single cached `Graphics` rect, not a separate texture per skill. **Asset sourcing:** primary = game file extraction via icon pipeline; fallback = synthetic WebP assets generated by a one-time Perlin noise script bundled as static resources. `background: 0x0a0a0b` in `app.init()` changes to fully transparent; a `backgroundSprite: TilingSprite` is inserted as the first child of `worldContainer` (before `edgeGraphics`).

~~OQ-5: **Weaver Tree data status**~~ — **RESOLVED:** The Weaver Tree tab currently shows a placeholder message ("data not available from community sources") — no interactive canvas exists yet. FR-F6 ships in two tiers: (1) **Phase 3:** apply the void/crystalline purple background to the `WeaverTreePlaceholder` panel itself, so the tab looks intentional and on-brand rather than a blank grey box; (2) **When community node data becomes available** (re-evaluate at start of Epic F): swap the placeholder for a real PixiJS canvas with the `bg_weaver_tile.webp` TilingSprite background already in place. No FRs are blocked — Epic F proceeds on the placeholder-panel approach and upgrades automatically when data arrives.

...
- [x] [Review][Patch] Clamp `armor_mitigation_percent` to a sane floor — `armor / (armor + 1104.0)` had only an upper `.min(0.85)`; a net-negative armor sum yielded a negative mitigation, and `armor == -1104` divided to `-inf` (serializes to a non-finite display value). Changed to `.clamp(0.0, ARMOR_MITIGATION_CAP)`. [lebo/src-tauri/scoring-core/src/compute/defense.rs:181-182] — Low — FIXED
- [x] [Review][Patch] Pin the denominator in `armor_mitigation_is_derived_and_caps_at_85` — the test fed `armor == ARMOR_MITIGATION_DENOM_REF_L100` (1104), so `A/(A+A)=0.5` passed for *any* K. Now feeds `armor = 368` → expects 25%, so the test fails if K changes. [lebo/src-tauri/scoring-core/src/compute/mod.rs] — Low (test quality) — FIXED
- [x] [Review][Patch] Add an AR-14 epsilon-band test case — `floor_check_resistance_at_cap_emits_no_warning` only tested 74.999999 (gap ~1e-6), which any positive epsilon suppresses. Added an in-band case (74.97 → no warning) and a just-outside case (74.9 → warns, gap ≈ 0.1) to pin `RESISTANCE_GAP_EPSILON = 0.05`. [lebo/src-tauri/scoring-core/src/compute/mod.rs] — Low (test quality) — FIXED
- [x] [Review][Patch] Document the intentional `HEALTH/HP/LIFE`-before-`HEALING` precedence — added a comment noting a Health+Healing node maps to HP under the first-match-wins single-key model (current data has 0 such nodes; the 4th HEALING tag is the Healing+Damage node routed to damage). [lebo/src-tauri/src/services/game_data_loader.rs:335-344] — Low — FIXED

_All 4 patches applied and verified: `cargo test -p scoring-core` → 97 passed, 0 failed (parity gate held)._

...
## Deferred from: code review of 2-1-design-token-reconciliation (2026-06-04)

- **[✅ RESOLVED 2026-06-04] Verify rarity colors against the actual game** [`rarityColors.ts`, `global.css`, ADR-P4-007] — The reconciled rarity palette (normal `#C6C0B5`, magic `#4A7A9E`, rare `#C9A84C`, set `#5EBD78`, unique `#D4805A`, legendary `#B068E8`) was lifted from the **prototype designer's mockup** (`UI-Handoff/styles.css` `--rarity-*`), adopted via ADR-P4-007 as the "Claude Design palette" — **never sampled from Last Epoch's in-game UI**. This story even replaced a `/* Last Epoch canonical */` comment with `/* Claude Design palette */`. A companion app should match the game's rarity colors so players aren't confused. **Alec's call (2026-06-04): verify against the game first.**
  - **Rip attempt outcome (2026-06-04):** asset-ripping CANNOT recover these. LE is an **IL2CPP build** (no `Assembly-CSharp.dll`; `GameAssembly.dll` + `global-metadata.dat`). The authority is `EpochColorManager` (`Assets/Scripts/UI/Colors/EpochColorManager.cs`, fields `normalNameColour`/`magicNameColour`/`rarityColour`/`rarityColorSettings`, `GetTooltipNameColor`); its instance lives in `level1` (MonoBehaviour pathID 13361, 64 bytes) and only carries the string `"MasterColorList"` — which is NOT an asset name in any of the 573 bundles nor an Addressable in `catalog.bin`. The actual `Color` values are field-initializer defaults baked into native `GameAssembly.dll`; UnityPy can't read IL2CPP MonoBehaviour fields by name. Rarity enum confirmed: NORMAL/MAGIC/RARE/EXALTED/UNIQUE/SET/LEGENDARY.
  - **Resolution:** Captured from **LastEpochTools.com** (faithful LE reproduction; the running game is IL2CPP so colors aren't asset-rippable — see [last-epoch-il2cpp-rarity-colors memory]). Verified LE palette applied to `rarityColors.ts` + `--color-rarity-*` + tests: `common #F4F4F4 · magic #3096D2 · rare #E3D057 · set #6ADA76 · unique #BB5D0B · exalted #A672DB · legendary #E12166`. The designer "Claude palette" was wrong on most tiers (legendary purple→red, magic, unique, common; exalted corrected too). tsc/tests(50)/build green. ADR-P4-007 AR-9 annotated. **Tiny open item:** `rare #E3D057` came from a JPEG capture (±a few values) — refine from a desktop PNG if pixel-perfection is wanted. Side benefits to check during reconciliation: the `rare ≡ --color-accent-gold #C9A84C` collision (rare items blend into chrome) and the `legendary/exalted/node-suggested` 3-purple cluster. Update ADR-P4-007's AR-9 table to the verified values once captured.
- **Two-sources-of-truth: `--color-rarity-*` duplicate `rarityColors.ts` with no sync guard** [`global.css:47-53`] — the seven `--color-rarity-*` CSS tokens are currently **unused** (zero `var(--color-rarity-*)` consumers in `src/`; live rarity colors flow through `rarityColors.ts`). AC3 updated their values to mirror the TS constant, but nothing enforces the mirror — a future edit to one file drifts silently. Consider a sync-guard test (assert CSS vars === `RARITY_COLORS`) or deleting the unused tokens in a future cleanup.
- **`getRarityColorForItemType` covers only `base`/`unique`; 5 tiers untested via a consumer** [`rarityColors.ts:13-14`] — the function signature is `'base' | 'unique'`, so magic/rare/set/exalted/legendary recolors are asserted only at the constant, never through an item-rendering path. Pre-existing, signature-limited.
- **PixiJS renderer hardcodes inline node-state hexes incl. `#c9a84c`** [`pixiRenderer.ts:418-426`] — skill-tree node-state colors (`#c9a84c` allocated, etc.) are inline and route through no token; `#c9a84c` now coincidentally equals the new `rare` rarity value. Not a rarity color by intent, pre-existing, out of this re-skin's scope — the renderer routes none of its colors through `rarityColors.ts`. Flag for the broader PixiJS token-adoption cleanup.

## Deferred from: code review of 2-3-left-panel-build-identity-and-section-navigator (2026-06-04)

- **Hardcoded `rgba(201,168,76,…)` / `rgba(255,255,255,0.06)` literals instead of `var(--color-*)` tokens** [`LeftPanel.tsx:129,177-178`] — active-row background (`rgba(201,168,76,0.12)`) and border (`rgba(201,168,76,0.25)`), plus the active-build card hairline (`rgba(255,255,255,0.06)`), use raw gold/white alpha literals where Pattern P4-8 wants `var(--color-*)` tokens. `#C9A84C = rgb(201,168,76)` is accent-gold, but `global.css` defines no accent-gold **alpha** variant (only solid `-soft`/`-dim`), so there is no drop-in token. Consistent with the same literal in `AppHeader.tsx:74` and `CenterCanvas.tsx:99` — a codebase-wide pattern, not a regression introduced by this story. Belongs to a token-system coverage pass (add `--color-accent-gold-alpha-*` tokens) rather than Story 2.3.

## Deferred from: code review of 2-4-right-panel-score-gauge-archetype-optimizer-chrome (2026-06-05)

- **Composite computed two different ways** [`RightPanel.tsx:69` vs `ScoreGauge.tsx:20-27`] — the new gauge `computeComposite` averages only the **non-null** sub-scores; the pre-existing collapsed-rail glyph divides by 3 treating `null` as 0. On a partial-null score (e.g. one axis uncomputed) the rail and the gauge display different composites. The rail line is unchanged context (pre-existing, not introduced by this story). Reconcile to a single shared composite helper when the rail is next touched.
- **`getArchetypeZone` has no input-domain guard** [`archetypeZone.ts:10-44`] — every comparison is `position < N`, so `NaN` falls through to "Glass Cannon" (most-aggressive) and out-of-range/fractional values pass silently. The store clamp (`optimizationStore.ts:136`, `Math.max(0, Math.min(100, pos))`) returns `NaN` unchanged and doesn't coerce fractional restores. Low risk — the normal slider path (`step=1`, ±5 keys) yields clamped integers; only a corrupted persisted `sliderPosition` reaches the gap. Add a `Number.isFinite`/round guard if the restore path is ever fed untrusted state.
- **Gauge center number and `ScoreTrio` pills are unclamped while the arc clamps to 0–100** [`ScoreGauge.tsx:115`, `ScoreTrio.tsx` `formatPill`] — `pct` clamps for the arc sweep, but the printed center composite and the DMG/SURV/SPD pill values are `Math.round` only. If the scoring engine ever emits out-of-range sub-scores, the text (e.g. "142") and the full arc disagree. `BuildScore` permits any `number`; only the `@deprecated` `scoringEngine.ts` clamps. Hardening only — relies on the live scoring path producing 0–100.
- **Delta indicator direction conveyed by ▲/▼ glyph + color only** [`ScoreGauge.tsx:123-134`] — the delta span carries no text alternative, so a screen reader gets an ambiguous "▲ 2.0" (the numeric value is still announced). Minor a11y enhancement — add an `aria-label` (e.g. "improves by 2.0" / "drops by 2.0") when the gauge a11y is next polished.

## Deferred from: code review of 2-5-center-canvas-six-tab-bar-with-weaver (2026-06-05)

- **LeftPanel nav highlights raw `centerTab`, not a `safeCenterTab` guard** [`LeftPanel.tsx:80`] — `CenterCanvas` (this story) added a `safeCenterTab` fallback that routes an invalid `centerTab` to `'tree'`, but `LeftPanel`'s nav-row `isActive = centerTab === row.id` still reads the raw value. On an invalid `centerTab`, `CenterCanvas` would highlight "Passive Tree" while `LeftPanel` highlights nothing — the two disagree. Unreachable today (`setCenterTab` is typed and `appStore` is not persisted, so `centerTab` can never hold an out-of-union value); fix is a one-line shared guard if defensive consistency is wanted when `LeftPanel` is next touched.

## Deferred from: code review of 2-6-blessing-editor-card-grid (2026-06-05)

- Stale/orphan `selectedId` becomes an invisible, unreachable selection (`BlessingsPanel.tsx:69,88`) — if `blessings[timelineId]` holds an id absent from the reloaded `blessingsDatabase` (or its timeline was dropped from the data), the None row is `isActive=false`, no entry row matches, and `hasActive=false`, so the card shows nothing selected even though the build still carries a non-null blessing id that scoring/serialization reads. The old `<select>` was equally blind to a stale value; `setBlessing` id-validation / orphan reconciliation is blessing data wiring — explicitly out of scope for this visual-rebuild story (AC4 / Scope Boundary). Address when a data-reconciliation/staleness story touches the blessing write path.
- `formatStatEffects` mis-renders non-`increased`/non-`flat` modifier types (`BlessingsPanel.tsx:15-22`) — `'+'` is added only for `increased` while `'%'` is added for any non-`flat`, so a `more` modifier renders `30% …` (no sign, no "more"), losing the distinction. Verbatim pre-existing format rule lifted from the old active-blessing summary; latent (0 `more` rows in `blessings.json` today).
- `formatStatEffects` produces `+-10%` / `+0%` for negative/zero values (`BlessingsPanel.tsx:19`) — hardcoded `'+'` for `increased` ignores value sign. Same verbatim pre-existing rule; no negative/zero blessing data exists today.

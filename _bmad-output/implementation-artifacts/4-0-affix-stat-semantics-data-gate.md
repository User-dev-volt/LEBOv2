...
- **AC2 — coverage gate + manifest (`check_coverage.py` + `affix-manifest.json`):** committed script, fails on any unmet gate, prints actual counts. Picker-visible = statKey + real name + non-empty slots. Results: **1,117 affixes, 508 statKey-resolved (45.5%), 508 picker-visible ALL clean (0 missing slots, 0 missing key, 0 `unnamed-*`), 55 distinct statKeys 0 dead, 2,338 unique mods 0 dangling, 821/897 bases with implicits, 30 implicit-deferral classes.** Manifest lists the 570-affix unmapped tail + inert keys + authored-slots provenance.
- **D4 — the ≥95%-of-total gate is INFEASIBLE and was reframed (surfaced to Alec, non-blocking).** The engine models ~55 consumable stats; ~54% of the LE affix corpus is unmodeled mechanics (ailment *chances*, conversions, "damage taken as", Haste buffs, potions, `+All Attributes`, Elemental Resistance) or engine-inert keys. Emitting any of them would ship a **dead key** (the exact Source-Audit defect this story fights). Real gate: **picker-visible affixes 100% clean** (met) + a 40% regression floor + the full unmapped list in the manifest. The 508 picker-visible affixes are 100% correct; that is the honest product state.
...
- **The no-dead-key guard did real work — it caught and forced honest exclusion of 13 keys** the create-story audit didn't anticipate: `added_*_damage` ×5 (engine models % increased, not flat added base — `FlatAdded*` never queried), `max_mana`/`mana_regen_per_sec`/`cooldown_recovery_speed` (unqueried), `increased_minion_count`/`increased_minion_hp` (`compute_minion` hardcodes them 0.0), `stun_chance` (inert). Also aligned crit chance/multiplier to **Flat-only** emission (offense.rs consumes Flat crit only → "increased crit" is inert) and coerced the damage-family `flat`→`increased` (a "+X% Fire Damage" affix is % increased). All recorded in the manifest.

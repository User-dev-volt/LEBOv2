...
- [x] [Review][Patch] Strengthen source-tracking test coverage [scoring-core/src/compute/mod.rs test module] — **APPLIED 2026-06-04.** Added 4 tests: `stat_sources_records_one_per_allocated_point` (a: 3-point node → 3 sources, value/label/modifier_type each asserted), `stat_sources_idol_and_gear_carry_value_and_modifier_type` (b: idol=12.0/gear=16.0 tier-resolved values + Flat asserted), `stat_sources_groups_same_origin_contributors` (c: two passive nodes → 2 distinct-label sources under one key), `loop_path_records_no_sources_in_registry` (d: asserts the false path records zero sources on the registry itself, not just the output flag). Gate: `cargo test -p scoring-core` **134/134** + `ehp_reference` 4/4.

...
- [ ] [Review][Decision] Attribute loader branches precede HEALTH/ATTACK_SPEED — latent frozen-gate risk for future co-tagged data — The 5 attribute branches sit at `game_data_loader.rs:316-320`, **above** ATTACK_SPEED (`:335`) and HEALTH (`:346`). First-match-wins, so a *future* node co-tagged `["VITALITY","HEALTH"]` would route to `Vitality` instead of player `MaxHp` (silently re-baselining `effective_hp` — the story's central frozen gate), and `["ATTUNEMENT","ATTACK_SPEED"]` would drop the speed key. **No such node ships today** (audit + golden-count 203 confirm only standalone attribute tags + the one `["SPELL","INTELLIGENCE","DAMAGE"]` node, which the DAMAGE branch catches first), so there is zero current perturbation. The author documented the DAMAGE-precedence case (`:311-315`) but not the HEALTH/speed case. Resolution is a precedence design choice → see options at HALT below.
- [ ] [Review][Patch] Missing NaN/Inf guard on attribute + minion sums (inconsistent with ailment) [`compute/attributes.rs:11-13`, `compute/minion.rs:16-20`] — `ailment.rs::sum` (`:12-19`) deliberately guards `is_finite()` ("bad data must not poison the sheet"), but the sibling sums in `attributes.rs` and `minion.rs` (`minion_damage_multi`) do not. A non-finite modifier value would serialize as JSON `null` (serde_json), silently breaking the TS `number` contract. Fix: mirror the guarded `sum` (ideally extract one shared helper).
- [ ] [Review][Patch] Minion delivery-type match is raw `== "minion"`, not trim/case-folded [`compute/minion.rs:43`] — Every other delivery-type comparison in the codebase trims + lowercases (`modifier.rs` `from_data_str` pattern); this raw `== Some("minion")` would miss `"Minion"`/`" minion"`, hiding the Minion sub-sheet for a genuine minion build. Fix: `as_deref().map(|s| s.trim().eq_ignore_ascii_case("minion")) == Some(true)`.

...
- [x] **Task 1 — `buildStore.removeAllPoints` store action (AC1, AC2, AC4, AC6, AC7)**
  - [x] 1.1 Extend `ApplyNodeResult` in `shared/types/build.ts:81` → `{ success: boolean; error?: string; blockedByDependents?: string[]; removed?: string[] }` (additive, back-compatible).
  - [x] 1.2 Add to the `BuildStore` interface (`buildStore.ts`, near `fillNodeToMax` decl `:34-37`): `removeAllPoints: (nodeId: string, treeData: TreeData) => ApplyNodeResult`.
  - [x] 1.3 Implement mirroring `fillNodeToMax`'s shape (`:253-321`) but **inverted** (see Dev Notes §removeAllPoints). No build → `{ success: false }` (do NOT auto-create — you cannot remove from nothing). Unallocated node (`(nodeAllocations[nodeId] ?? 0) <= 0`) → `{ success: false }` **before** snapshotting (AC6).
  - [x] 1.4 Compute `orphans = computeOrphanedNodes(nodeId, activeBuild.nodeAllocations, treeData)`; build ONE `newNodeAllocations` deleting `nodeId` + every orphan (zero-key-deletion idiom, never store `0`); push exactly ONE snapshot via the P4-7 tail (`:318-319`); clear `redoStack`; set `updatedAt: new Date().toISOString()`. Return `{ success: true, removed: [nodeId, ...orphans] }`.
  - [x] 1.5 Do **not** add `isPersisted: false` (match `fillNodeToMax`/`applyNodeChange` passive convention — see Dev Notes §Traps).

- [x] **Task 2 — `computeOrphanedNodes` pure helper (AC2, AC7)**
  - [x] 2.1 New file `lebo/src/features/skill-tree/computeOrphanedNodes.ts`, named export `computeOrphanedNodes(nodeId, nodeAllocations, treeData): string[]` (orphan ids, **excluding** `nodeId`). Pure — no store/React import (mirrors `nearestAllocatedPath.ts`).
  - [x] 2.2 Implement the **strict `.every()` root-reachability** algorithm (Dev Notes §Algorithm), O(V+E): build directed adjacency once; seed surviving roots (no prerequisites); a dependent becomes *valid* only when **all** its prerequisites are valid; orphans = surviving − valid. Cycle-safe via visited set + head-cursor (no `Array.shift`).
  - [x] 2.3 Both `removeAllPoints` (Task 1) and the AC2 pre-check in the hook (Task 3) call it — keep it a standalone pure helper, not inline store logic.
  - [x] 2.4 **Data check (D3):** grep shipped `resources/game-data/classes/*.json` for any node with `prerequisiteNodeIds.length > 1`. Record the finding in the Dev Agent Record. If none exist, note that AC2's diamond case is covered by synthetic fixtures only (the algorithm must still be correct).

- [x] **Task 3 — Add the shift+right-click → remove-all gesture (AC1, AC2, AC3, AC5, AC8)**
  - [x] 3.0 **Gesture model (Alec-confirmed, D2):** **right-click = remove ONE** (existing `applyNodeChange(-1)`, incl. its blocked-with-flash guard — UNCHANGED, AC3); **shift+right-click = remove ALL** + orphan cascade. Remove-all rides the **existing `onNodeClick(nodeId, button, shiftKey)` plumbing** (button===2 + shiftKey) exactly like 3.4's shift+left fill. The inert `onNodeContextMenu` / `NodeContextMenu.tsx` are NOT used.
  - [x] 3.1 `pixiRenderer.ts:618-621` (WebGL hit-area `pointerdown`): change `callbacksRef.current.onNodeClick(node.id, 2)` → `callbacksRef.current.onNodeClick(node.id, 2, e.shiftKey)` — forward `shiftKey`, exactly as the `pointerup` left-click already does (`:627`). Native context menu is already suppressed (`pixiRenderer.ts:207-209`), so shift+right won't pop the OS menu.
  - [x] 3.2 `SkillTreeCanvas.tsx:358-361` (focused-node DOM `onContextMenu`, a11y/keyboard parity): change `onNodeClick(id, 2)` → `onNodeClick(id, 2, e.shiftKey)` (keep `e.preventDefault()`). This is how a keyboard user reaches remove-all (Shift + context-menu key).
  - [x] 3.3 `useSkillTree.ts` `handleNodeClick` (`:59-88`) — add a **shift+right-click passive branch**, placed beside the existing shift+left fill branch (`:64-71`):
    ```ts
  ...
  - [x] 3.4 Add hook state `pendingRemoval: { nodeId: string; orphanIds: string[] } | null` + `confirmRemoval()` (calls `removeAllPoints(pendingRemoval.nodeId, treeData)`, then clears) + `cancelRemoval()` (clears); return all three from the hook. No coords needed — the confirm is a centered modal.
  - [x] 3.5 **No skill/weaver rewiring.** Because remove-all rides `onNodeClick` (not `onNodeContextMenu`) and is passive-guarded (`slotId === undefined`): on a skill slot, shift+right falls through to single `applySkillNodeChange(-1)`; the weaver uses its own local `handleWeaverNodeClick` (`SkillTreeView.tsx:525-537`) which ignores `shiftKey` → weaver shift+right = single `applyWeaverNodeChange(-1)`. Verify these still behave (test row h); change nothing in the weaver/skill wiring.

- [x] **Task 4 — AC2 confirmation dialog (AC2)**
  - [x] 4.1 New `lebo/src/features/skill-tree/RemoveNodeConfirmDialog.tsx`, a centered modal mirroring the **`DeleteConfirmDialog` pattern** (`build-manager/DeleteConfirmDialog.tsx` — Headless UI `Dialog`/`DialogPanel`/`DialogTitle` + dimmed backdrop + Cancel/Continue pair). **Do NOT import build-manager's component** (no cross-feature imports — project rule); replicate the pattern locally. Props: `{ orphanNames: string[]; onConfirm: () => void; onCancel: () => void }`.
  - [x] 4.2 Body text names the orphans exactly: `Removing this node will also deallocate: ${orphanNames.join(', ')}. Continue?` (AC2 literal). The dialog receives **already-resolved** `orphanNames: string[]` (resolution happens in SkillTreeView, 4.3).
  - [x] 4.3 Mount in `SkillTreeView.tsx`: destructure `pendingRemoval`/`confirmRemoval`/`cancelRemoval` from `passiveInteraction` (`:482` / `:486-501` selection); render `<RemoveNodeConfirmDialog>` when `pendingRemoval` is set. **Resolve orphan ids → names with the REAL `getNodeName` signature** `getNodeName(id, gameData, classId, masteryId)` — SkillTreeView has `gameData` (`:84`), `selectedClassId` (`:88`), `selectedMasteryId` (`:89`): `const orphanNames = pendingRemoval.orphanIds.map(id => getNodeName(id, gameData, selectedClassId, selectedMasteryId))`. ⚠️ `treeData`/`TreeNode` carry **no** `name` field — names come **only** from `gameData`; never `getNodeName(treeData, id)`. `onConfirm` → `confirmRemoval()`, `onCancel` → `cancelRemoval()`. Focus ring + reduced-motion + axe per a11y rules.

- [x] **Task 5 — Tests (value + element)** — see Dev Notes §Testing for the full matrix.
  - [x] 5.1 `computeOrphanedNodes.test.ts` (NEW): chain, diamond (strict, b1/b2), leaf, root, multi-hop, root-never-orphaned, unallocated-input — exact id-set assertions.
  - [x] 5.2 `buildStore.test.ts` — new `describe('buildStore — removeAllPoints')` mirroring the `fillNodeToMax` block (`:502-585`): exact surviving map, `removed` set, `undoStack.length === 1`, `redoStack` cleared, `undoNodeChange()` one-step restore, no-op on unallocated, no `isPersisted` flip.
  - [x] 5.3 `useSkillTree.test.ts` — **ADD** shift+right-click tests; **KEEP** the existing plain-right-click decrement tests (`:72-78` "removes one point", `:161-168` "blocked-decrement flashes") — AC3 keeps them valid. New cases (mirror 3.4's shift+left routing test `:178-211`): passive `handleNodeClick(id, 2, true)` no-orphans → exact-removed post-state + `undoStack 1`; with-orphans → sets `pendingRemoval`; **skill slot** `handleNodeClick(id, 2, true)` → single `applySkillNodeChange(-1)`, passive map untouched (passive guard falls through).
  - [x] 5.4 `SkillTreeCanvas.test.tsx` — **update** the standing-RED `:120` test to the gesture contract: `onContextMenu` fires `onNodeClick(id, 2, e.shiftKey)` (shiftKey forwarded), NOT `onNodeContextMenu`. Assert both plain (shiftKey=false) and shift (shiftKey=true) forward correctly. (The old menu-path expectation is obsolete under the gesture model.)
  - [x] 5.5 `RemoveNodeConfirmDialog.test.tsx` (NEW): renders exactly the orphan names (set-equal, no extras/omissions); Continue → `onConfirm` once; Cancel → `onCancel`, allocations untouched; axe clean.
  - [x] 5.6 `SkillTreeView.test.tsx` — weaver shift+right-click does **not** remove-all: decrements via `applyWeaverNodeChange(-1)`, passive `nodeAllocations` untouched, no `removeAllPoints` call (row h guard).

- [x] **Task 6 — Verify & baseline**
  - [x] 6.1 `pnpm build` (tsc + vite) clean.
  - [x] 6.2 Full `pnpm vitest`: **no new failures vs the standing 8-failure baseline** (`ProviderSelector` / `Settings` / `SkillTreeCanvas` / `TreeControls`; HEAD after 3.3+3.4 = **1237 passed / 8 failed**). Reworking `SkillTreeCanvas.test.tsx:120` to the gesture contract should make it pass — note whether that **clears SkillTreeCanvas from the baseline** or whether other failures in that file remain. Net test count should rise. Do not "fix" unrelated baseline failures.
  - [x] 6.3 Re-Read+grep the passive guard (`useSkillTree.ts` `slotId === undefined`) and `removeAllPoints` body on the **working tree** post-edit (AutoSave-anomaly guard).
...
_bmad-code-review, 3 adversarial layers (Blind Hunter / Edge Case Hunter / Acceptance Auditor — all Opus 4.8), diff `c34761c..HEAD` scoped to the 14 story-3.5 files. **Acceptance Auditor: AC1–AC8 all MET.** Strict-`.every()` cascade (D3) and single-undo snapshot (AC4 / Pattern P4-7) independently source-verified against the live store. Triage: **1 decision-needed, 0 patch, 2 deferred, 3 dismissed.**_

### Decision-needed

- [x] **[Review][Patch] AC2 confirmation dialog is not modal against window-level keyboard shortcuts** (MEDIUM) — **✅ FIXED 2026-07-01 (approach 1)** [`App.tsx:121-132,150-155`; `useSkillTree.ts:161-167`; `SkillTreeView.tsx:913`] — App.tsx's `window` keydown handler fires Ctrl+Z/Y undo/redo and bare center-tab keys 1–6 **while `RemoveNodeConfirmDialog` is open**, because its guards only skip input/textarea/contenteditable targets, not an open app-modal (Headless UI traps focus but not these window listeners). Two manifestations: **(a)** a mid-modal undo/redo mutates `nodeAllocations`, so `confirmRemoval`'s recompute-at-confirm `removeAllPoints(nodeId, treeData)` can deallocate a set that **differs from the orphans the dialog named** (breaks AC2 exact-naming) — or silently no-op if the target was undone — and clears `redoStack`, discarding the just-performed redo; **(b)** a bare 1–6 keypress switches center tab, unmounting the passive-branch dialog while `pendingRemoval` stays armed (no unmount/tab-change cleanup), a phantom pending-confirm that re-appears on return and acts on later state. Verified reachable in real source; no data corruption / no invalid allocation state. Fix options — **(1, recommended)** suppress the App.tsx undo/redo + 1–6 shortcuts while a confirm modal is open (shared modal-open flag) **and** clear `pendingRemoval` on tab-change/unmount; **(2)** reconcile at confirm — re-derive orphans and abort/re-show if they differ from `pendingRemoval.orphanIds`; **(3)** accept as-is (recompute-at-confirm never yields an invalid state; requires an unusual mid-modal keypress).

  **Resolution (approach 1):** Added `appStore.isBlockingModalOpen`, set while a passive remove-all confirm is open and cleared on unmount (a `SkillTreeView` effect driven by `passiveInteraction.pendingRemoval`). `App.tsx`'s `window` keydown handler now early-returns when the flag is set, so undo/redo + the 1–6 tab keys no longer fire under the dialog (Escape and Ctrl+S remain allowed). Added a defense-in-depth effect that drops a pending removal when the passive tab is left. New regression tests in `App.keyboard.test.tsx` (tab-switch suppression, undo/redo suppression, resume-on-close). Verified: `pnpm build` clean; full `pnpm vitest` **1273 passed / 7 failed** — exactly the standing baseline (ProviderSelector×5 / Settings×1 / TreeControls×1), **+3 tests, no regressions**. Files: `appStore.ts`, `App.tsx`, `SkillTreeView.tsx`, `App.keyboard.test.tsx`.

  **Also verified LIVE in the real Tauri app (2026-07-01)** via a new reusable CDP runner (`scripts/tauri-runner/`, driving the actual WebView2 over `--remote-debugging-port`): loaded the `Bladedancer 101` build, shift+right-clicked a hub node → the AC2 dialog named its 6 real orphans (Dexterity, Nimble, Evasion, Ambush, Poison Expertise, Shadow Step); pressing **Ctrl+Z** and the **`2`** tab-key under the open dialog left it fully intact (dialog stayed mounted, build unchanged at 26 pts) — proving the suppression works end-to-end; **Escape** then cancelled cleanly (nothing removed) and pressing `2` afterward correctly switched to the Weaver tab (shortcuts resume once the modal closes).

### Deferred

- [x] **[Review][Defer] `computeOrphanedNodes` over-orphans nodes reachable only via a dangling / self-loop / rootless-cycle prerequisite edge** [`computeOrphanedNodes.ts:33-37`] — `prereqCount` counts every incoming edge, so a surviving node whose prerequisite source is absent from the loaded `treeData.nodes` (or sits in a rootless cycle) never clears its counter and is falsely deallocated on ANY `removeAllPoints`. Unreachable in curated DAG data (the strict-AND `.every()` allocation gate at `buildStore.ts:211/299/369/525` makes such an allocation impossible to build); realistic only via a mismatched-mastery/imported build (unimplemented Epic 7 import). Deferred — pre-existing data-integrity assumption; revisit when build-import lands (validate/prune dangling allocations).
- [x] **[Review][Defer] Orphan id→name resolution in SkillTreeView has no integration test** [`SkillTreeView.tsx:~915`] — `RemoveNodeConfirmDialog.test.tsx` asserts the named set only with pre-resolved `orphanNames`; the actual `pendingRemoval.orphanIds.map(id => getNodeName(id, gameData, selectedClassId, selectedMasteryId))` wiring in SkillTreeView has no regression guard. Source verified correct (full `getNodeName` signature, arrow form — avoids row-g's warned-against bare `.map(getNodeName)`). Deferred — test-hardening only.

### Dismissed (noise / false positive / handled elsewhere)

- **[Blind Hunter, HIGH] `computeOrphanedNodes` over-refunds if allocation is OR-reachability** — FALSE POSITIVE. The allocation gate is strict-AND `.every()` at all four sites (`applyNodeChange:211`, `fillNodeToMax:299`, weaver `:369`, skill `:525`); a node cannot be validly allocated without ALL prerequisites, so the over-refund state is unreachable, and the 32 real diamond nodes (Task 2.4 — e.g. `void-sword` requires `rift-strike` AND `consume-void`) are genuine AND-prerequisites. The strict cascade is correct-by-design and matches D3 (Alec-confirmed). The Blind Hunter was blind to the allocation semantics (it flagged this conditionally).
- **[Acceptance Auditor] Diff bundles already-reviewed 3.3+3.4 focus/tooltip carryover (touches `optimization:*`)** — HANDLED ELSEWHERE. The `focusNode(): boolean` retry + suggestion-tooltip lifecycle hunks are the 3.3+3.4 code-review follow-up patches, already applied & reviewed (`sprint-status.yaml` → `last_updated_review_3334`), co-mingled into the 2026-06-30 AutoSave window. Not a 3.5 defect (the two pure-carryover files `types.ts`/`pixiRenderer.test.ts` were already excluded from the review diff). Recurring `[[autosave-watcher-unvalidated]]` commit co-mingling — commit-hygiene note, not a code change.
- **[Acceptance Auditor] Weaver row-h proven by outcome, not a call-spy** — informational; the assertions are sound (`removeAllPoints` is passive-only and never touches `weaverAllocations`).

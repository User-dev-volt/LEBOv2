...
- [ ] **Task 3 — Rewire passive right-click → remove-all (AC1, AC2, AC4, AC7)** *(Option A — see D1)*
  - [ ] 3.1 `pixiRenderer.ts:618-621`: replace `callbacksRef.current.onNodeClick(node.id, 2)` with `callbacksRef.current.onNodeContextMenu?.(node.id, e.client.x, e.client.y)`. **Use DOM client coords (`e.client`), not `e.global`** (stage coords) — the confirm/menu is `position: fixed`.
  ...
  - [ ] 3.3 `useSkillTree.ts` — repurpose `handleNodeContextMenu(nodeId, x, y)`: **passive** (`slotId === undefined`) → compute orphans; empty → `removeAllPoints(nodeId, treeData)` immediately (AC1); non-empty → set a `pendingRemoval` state `{ nodeId, orphanIds, x, y }` (AC2). **Skill slot** (`slotId !== undefined`) → preserve current single-decrement (`applySkillNodeChange(slotId, nodeId, -1, treeData)` + existing flash-on-block). Expose `pendingRemoval` + a `confirmRemoval()` + `cancelRemoval()` (replacing the now-unused `contextMenu`/`handleContextMenuClose`).
  - [ ] 3.4 `handleNodeClick`: the `button === 2 ? -1 : 1` branch (`:73`) is no longer reached from the canvas — simplify to allocate-only or leave inert (dev discretion; keep `tsc` clean under `noUnusedParameters`). Do not break left-click (+1) or shift+left (fill, 3.4).
...
- [ ] **Task 4 — AC2 confirmation dialog (AC2)**
  ...
  - [ ] 4.2 Body text names the orphans exactly: `Removing this node will also deallocate: ${orphanNames.join(', ')}. Continue?` (AC2 literal; naming the node is optional polish). Resolve ids → names via `getNodeName(treeData/gameData, id)`.
  - [ ] 4.3 Mount in `SkillTreeView.tsx`: destructure `pendingRemoval`/`confirmRemoval`/`cancelRemoval` from the hook (currently omitted), and render `<RemoveNodeConfirmDialog>` when `pendingRemoval` is set. `onConfirm` → `confirmRemoval()` (calls `removeAllPoints`), `onCancel` → `cancelRemoval()`. Focus ring + reduced-motion + axe per project a11y rules.
...
- [ ] **Task 5 — Tests (value + element, AC1–AC7)** — see Dev Notes §Testing for the full matrix.
  ...
  - [ ] 5.3 `useSkillTree.test.ts` — passive right-click → remove-all + cascade-confirm flow; passive-only routing (skill slot still single-decrements); **replace** the old passive `right-click removes one point` / `blocked-decrement flashes` tests (`:72-78`, `:161-168`) — that behavior is superseded for passive.
...
| `lebo/src/features/skill-tree/SkillTreeView.tsx` | Consume `pendingRemoval`, mount dialog |
...
Flow lives in `useSkillTree.handleNodeContextMenu` → passive computes orphans → direct `removeAllPoints` (AC1) or `pendingRemoval` state → `<RemoveNodeConfirmDialog>` in `SkillTreeView` → `removeAllPoints` on confirm (AC2). Skill slots keep single-decrement.
...
| b | diamond `root→{A,B}→C`, allocate all, `removeAllPoints('A')` | **(strict, D3)** helper → `['C']`; `A`,`C` deleted, `root`,`B` remain; `undoStack 1`. Then `removeAllPoints('B')` → `['C']`-cleared too |
...
| g | confirm dialog naming | rendered orphan names set-equal `orphans.map(getNodeName)`; Continue → `onConfirm` once; Cancel → allocations untouched; axe clean |
| h | hook routing | passive right-click → remove-all/cascade; skill-slot right-click → still single `applySkillNodeChange(-1)`, passive map untouched |

...
| 2026-06-30 | Code review (bmad-code-review, ultracode: 6 parallel adversarial lenses — Blind Hunter / Edge Case Hunter / Acceptance Auditor ×2 / Source-Audit specialist / Render-correctness — over the combined 3.3+3.4 diff `9fd6030..c34761c`, each raw finding independently refuted by a skeptic with repo access). **4 raw → 2 confirmed / 2 refuted.** Confirmed (both → Patch): (1) **MEDIUM** — compact canvas tooltip fabricates a "0 pts" point cost and lingers when its card is skipped while the highlight is active (`SkillTreeView.tsx:864`; displayed-but-not-sourced / AC2); (2) **LOW** — `focusNode` no-ops on the first cross-tab card activation against a freshly-mounted passive canvas (`SkillTreeView.tsx:293`; AC3/AR-6, self-heals on re-hover). Refuted/dismissed: synthetic-card stale-highlight (dead Epic-6 scaffolding, unreachable from product) + `pathPointCost` no-anchor `0` (spec-sanctioned + test-pinned). Findings written to Review Findings below. Status pending patch decision. |
| 2026-06-30 | Review patches APPLIED (Alec: apply both). **①** Compact tooltip is now gated on a LIVE backing suggestion (`cardTooltip && cardTooltipNode && cardTooltipSuggestion`) and a new `SkillTreeView` effect keyed on `suggestions` clears the cross-highlight + `previewStatSheet` + `cardTooltip` when the backing suggestion is removed (skip / apply / re-optimize) — eliminates the fabricated "0 pts" and the lingering glow (the `: 0` / `: null` fallbacks dropped). **②** `focusNode` now returns a `boolean` (resolved vs not-ready); `SkillTreeView`'s focus effect retries across frames (cap ~60) until the freshly-mounted passive canvas's async renderer is ready, so the first cross-tab activation centres reliably (`types.ts` + `pixiRenderer.ts` + `SkillTreeCanvas.tsx` + `SkillTreeView.tsx`); +3 `pixiRenderer.test.ts` boolean-contract tests (unknown id → false, pre-resize → false, resolved+sized → true). Verified: `pnpm build` (tsc + vite) exit 0; full `pnpm vitest` **1237 passed / 8 failed** = exact standing baseline (ProviderSelector/Settings/SkillTreeCanvas/TreeControls), **+3 tests, no new failures**. Both patch items checked off. Status → done. |

---

### Review Findings

_Code review 2026-06-30 — `bmad-code-review` (ultracode): 6 parallel adversarial lenses over the combined 3.3+3.4 diff `9fd6030..c34761c`, each finding independently refuted by a skeptic with repo access. **4 raw findings → 2 confirmed, 2 refuted.** Story 3.4 reviewed **clean** in the same pass._

**Patch — confirmed, introduced by this story:**

- [x] [Review][Patch] (FIXED 2026-06-30) Compact card tooltip fabricates a "0 pts" point cost and lingers when its suggestion is removed (e.g. Skip) while the highlight is active [lebo/src/features/skill-tree/SkillTreeView.tsx:864] — **MEDIUM / source-audit (AC2 + Source-Audit mandate)**. Activate an actionable card (hover / click / ArrowDown → sets `highlightedNodeIds.glowing={X}` + `requestNodeFocus(X)`, so the compact tooltip shows for X), then Skip it. React fires no `onMouseLeave` on unmount, so `deactivate()` never runs: `highlightedNodeIds` keeps glowing X and the dismiss effect (clears only when `glowing.size===0`) never fires. `cardTooltipSuggestion = suggestions.find(toNodeId===X)` is now null, so `pointCost={cardTooltipSuggestion ? … : 0}` renders a fabricated "0 pts / N pts to reach" plus stale per-stat deltas until the user interacts elsewhere. Deterministic via ArrowDown → press 'S'. This is exactly the displayed-but-not-sourced class the story's own Source Audit (and the sibling `SuggestionsList` test) were written to prevent — that test only covers hovering an already-skipped card, not skipping a live one. **Fix:** clear the cross-highlight + `previewStatSheet` + `cardTooltip` when the suggestion backing `cardTooltip.nodeId` disappears (an effect keyed on `suggestions`, or clear inside the skip handler); drop the `: 0` fabricated fallback so no cost renders without a backing suggestion.
- [x] [Review][Patch] (FIXED 2026-06-30) First cross-tab card activation fails to center the node — `focusNode` no-ops on a freshly-mounted passive canvas [lebo/src/features/skill-tree/SkillTreeView.tsx:293] — **LOW / acceptance (AC3 / AR-6)**. When the passive canvas is unmounted (a skill/weaver tab, the "No skill selected" placeholder, or the full-panel skill picker) and a card is activated for the first time, the focus effect runs `setActiveTabIndex(0)` then a **single** `requestAnimationFrame` before `passiveCanvasRef.current?.focusNode(id)`. One rAF is not enough for the async `app.init()` + `Assets.load()`: `rendererRef.current` is still null (the handle no-ops) and `lastRenderedNodeMap` is empty (`focusNode` early-returns), so the off-screen node is not centered on that first interaction. The glow still applies (prop-driven) and it self-heals on re-hover (`focusNonce` re-fires). **Fix:** keep the focus request pending until the renderer is ready/has rendered (mirror `pixiRenderer.fitToTree`'s `pendingFitNodes` defer), or retry on a short rAF loop until `focusNode` resolves the node.

**Dismissed — raised but refuted on verification:**

- [Review][Dismiss] "Keyboard arrow-nav onto a synthetic card leaves the previous card's highlight/tooltip" (`SuggestionsList.tsx:224`) — **REFUTED**: the synthetic early-return does precede the highlight-clear, but synthetic (`warning:`/`unique:`/`synergy:`) cards are dead-but-keep Epic-6 scaffolding; `run_optimization` is `passive_node`-only (`scoring_commands.rs` + its regression test), so no synthetic card is ever in the live suggestions list. Unreachable from real product input.
- [Review][Dismiss] "`pathPointCost` returns 0 for nodes with no allocated anchor, hiding the real path cost" (`pathPointCost.ts:54`) — **REFUTED**: spec-sanctioned (Task 2 mandates mirroring `nearestAllocatedPath`'s clean no-op) and test-intended (`pathPointCost.test.ts:50-52` deliberately asserts `=== 0`); consistent with the dashed-line no-op; genuinely unreachable suggestions are independently blocked at apply time.

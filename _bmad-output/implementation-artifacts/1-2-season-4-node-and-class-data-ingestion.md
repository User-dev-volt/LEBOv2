...
- [x] [Review][Defer] S4 nodes use hard-coded pixel coordinates with no spatial collision check — verify visually when the tree renders [scripts/annotate_s4_nodes.py:S4_ADDITIONS] — deferred, placeholder data pending real coordinates from lastepochtools.com
- [x] [Review][Defer] `inject_s4_nodes` builds `existing_ids` once before the injection loop — duplicate IDs within a single S4_ADDITIONS entry would both be inserted; not triggered by current data [scripts/annotate_s4_nodes.py:inject_s4_nodes] — deferred, pre-existing theoretical edge case
- [x] [Review][Defer] `copy_bundled_resources` overwrites user-fetched remote data on any version mismatch — users who downloaded a newer remote patch would lose it on app relaunch [src-tauri/src/game_data_service.rs] — deferred, pre-existing behavior not introduced by this story
- [x] [Review][Defer] Permanent staleness bar if CDN is not updated to `"Season 4 (Shattered Omens)"` simultaneously — pure string equality means version never resolves until CDN is updated [src-tauri/src/game_data_commands.rs] — deferred, deployment/operational concern not a code bug

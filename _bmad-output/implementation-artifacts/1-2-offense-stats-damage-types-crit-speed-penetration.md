...
### Penetration Sourcing (added 2026-06-02 from code review — re-opened for dev)
...
| 2026-06-02 | Second adversarial code review (full story incl. pen sourcing): 1 decision-needed, 0 patch, 5 deferred, 7 dismissed; AC1–AC5 satisfied, no Critical/High regressions. Decision resolved by patch — `penetration.rs` reworked to per-element `PenByElement` + primary-type-count averaging (non-penetrable types dilute at ×1.0; each element gets only its own pen); fixes the hybrid/elemental over-count flagged at review request while staying forward-compatible with future damage-share weighting. scoring-core 91/91, loader golden 185 green. Status → done. |
...
### Review Findings — 2026-06-02 (second adversarial review, full story incl. penetration sourcing)

_Three parallel layers (Blind Hunter + Edge Case Hunter + Acceptance Auditor) over `git diff 61230f0..HEAD` (post-Story-1.1 baseline → both Story-1.2 dev passes). All claims verified at code level. Result: 1 decision-needed, 0 patch, 5 deferred, 7 dismissed as noise. AC1–AC5 satisfied at the literal-contract level; no Critical/High regressions._

- [x] [Review][Decision → PATCHED] Hybrid / elemental penetration → primary-type mapping is inconsistent under the equal-split model — `penetration_multiplier` (compute/penetration.rs:71-84) only pushes a channel for **penetrable** primary types, so a non-penetrable primary (necrotic/poison) is dropped from the average denominator instead of contributing a ×1.0 share: `["fire","necrotic"]` + 50 elemental pen returns **×1.50** when equal-split intent says **×1.25** (the necrotic half is unpenetrated). Meanwhile `["fire","physical"]` with fire-only pen correctly returns ×1.25 because `physical` is always a channel (pushed even at 0 pen). Second facet: `compute_penetration` (penetration.rs:37-40) **sums** fire+cold+lightning+generic-elemental into one figure and applies it whole whenever the primary includes *any one* elemental type — a fire-primary build with a stray cold-pen affix folds that cold pen into its multiplier (over-count). Both facets are inside the equal-split assumption Alec flagged at review request. **Decision:** fix the denominator now (count all primary damage types; non-penetrable types contribute ×1.0) and/or restrict the elemental figure to the primary's actual elements — or accept the simplification until real per-type damage-share data exists (then the whole thing becomes a weighted average, per Alec's note in penetration.rs). **Resolution (Alec, 2026-06-02): patched to the most accurate model achievable without damage-share data.** `compute_penetration` now returns a per-element `PenByElement`; `penetration_multiplier` averages over **all** primary damage types (non-penetrable types contribute an unpenetrated ×1.0 share, so `["fire","necrotic"]`+50 elem pen → ×1.25 not ×1.50) and applies each element only its own penetration + generic elemental (a stray cold-pen affix no longer inflates a fire-only build). The surfaced `elemental_penetration` AC4 field stays the fire/cold/lightning + generic sum via `PenByElement::elemental()`. The per-type structure is the equal-weight form of `Σ(shareᵢ × multᵢ)` — drop in real damage-share weights later with no restructure. 3 regression tests added (`hybrid_with_non_penetrable_type_dilutes_the_multiplier`, `elemental_pen_applies_only_to_its_own_element`, `generic_elemental_applies_to_each_elemental_primary`). Verified: `cargo test -p scoring-core` → 91 passed; `cargo test -p lebo game_data_loader` → 3 passed (golden 185). No TS/contract change.
- [x] [Review][Defer] Typed/DoT `More` damage mods surface in the per-type breakdown but never reach `damage_score` [compute/offense.rs:48-71] — deferred, pre-existing — the aggregate `more_factor` queries only `StatKey::MoreDamage` and `total_increased` filters `Increased`, so a `More`-typed `IncreasedFireDamage`/`IncreasedIgniteDamage` shows in `fire.more`/`more_dot` but does not move the score. Aggregate behavior is unchanged from Phase 3; the breakdown merely makes the gap *visible*. No current data path produces such a mod. Score/breakdown reconciliation is Story 1.6 UI concern. (Re-confirmed from first-pass review.)
- [x] [Review][Defer] `StunChance` is consumed but never sourced → `stun_chance` always 0 in production; `Flat`-only filter would also silently drop a future `Increased`-typed stun source [compute/offense.rs / game_data_loader.rs] — deferred, knowingly — no `tags_to_stat_key` STUN branch (wiring it breaks golden count); tracked in MEMORY `project_stun_chance_unsourced`. Same "computed-but-not-sourced" defect class that re-opened AC4 penetration; AC2 says "computed", so literal AC met, but FR-2 stun is functionally inert.
- [x] [Review][Defer] `parse_penetration_clause` prose parser is brittle — and the golden test cannot catch its failure modes [game_data_loader.rs:239-271] — deferred, new code but inert on shipped data — splits clauses only on `". "` (misses `".\n"`, `"; "`), takes only the **first** pen clause via `.find` (a node with two pen clauses drops the second), and resolves the element by first-substring-match (`"Fire and Lightning Penetration"` → fire only; `"void"` is a substring of `"avoidance"`). None trigger on shipped data (golden 185 passes), **but `shipped_class_json_effect_count_is_stable` counts effects only** — a mis-valued or mis-element pen leaves the count at 185 and is not caught. Recommend a value+element assertion test over the real pen nodes before relying on the parser for new data.
- [x] [Review][Defer] Single-stat path `extract_value` reads the first number in the whole description (assumes the damage clause leads) [game_data_loader.rs:212] — deferred, pre-existing — path (1) of `parse_node_effects` is byte-identical to pre-1.2 parsing; correct for shipped DAMAGE+PENETRATION nodes (damage clause first, verified by golden 185), but order-dependent. Relevant now that multi-clause pen nodes exist; a pen-first or lead-number-prose node would mis-value the damage key.
- [x] [Review][Defer] Per-type breakdown does not reconcile to the aggregate; IGNITE-before-FIRE bucket shift [compute/offense.rs:7-37] — deferred, by design — delivery/generic keys (`IncreasedSpellDamage`/`MeleeDamage`/`RangedDamage`/`AreaDamage`/`IncreasedDamage`) are in `DAMAGE_STAT_KEYS` (aggregate) but absent from the `DAMAGE_TYPES` table (breakdown), so Σ(breakdown.increased) < aggregate `total_increased` whenever such mods exist. Informational for any Story 1.6 UI that sums the breakdown. (Folds in the first-pass IGNITE/FIRE bucket-shift spot-check.)

**Dismissed as noise (7):** pen also scales `avg_hit_damage`/`avg_hit_damage_crit_weighted` (defensible & parity-safe — Phase-3 crit tests have empty primary → ×1.0); NaN penetration (not reachable from finite digit parsing); pen ≥ 100 unbounded (intentional per the spec's documented linear model); `ENEMY_RES_BASELINE=0` indirection (intentionally documents the 0%-reference-target assumption); `void_penetration` scope expansion beyond AC4 (justified, documented, no dead keys — both new keys produced AND consumed); unmodeled-elemental primary has no diagnostic (intended ×1.0 fallback); "aggregate parity untested on shipped data" (verified sound **by construction** — NECROTIC/BLEED/IGNITE all move between members of `DAMAGE_STAT_KEYS`, so Σincreased is invariant).

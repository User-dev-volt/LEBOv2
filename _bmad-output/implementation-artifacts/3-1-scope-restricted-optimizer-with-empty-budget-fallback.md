...
_Code review 2026-06-29 (bmad-code-review — 3-layer adversarial: Blind Hunter + Edge Case Hunter + Acceptance Auditor, all Opus). **All 3 ACs verified MET; no Critical/High; no "Do not touch" boundary crossed.** 4 findings dismissed as noise/false-positive/spec-sanctioned: (a) vacuous Rust off-tree assertions — documented tradeoff, genuine positive element-level coverage remains; (b) guard integration seam untested — mitigated, real `selectUnspentPassivePoints` covered in `buildStore.test.ts`; (c) real selector "missing" null-fallbacks — false positive, `BuildState.characterLevel`/`nodeAllocations` are required (build.ts:45,47); (d) AC1 enforced at the Claude payload not the streamed NDJSON — spec-sanctioned (story line 151), payload built only from knapsack passive nodes._

### Resolved — Patched (2026-06-29 code review)

- [x] [Review][Patch] Empty-budget notice goes stale after a same-build edit — **RESOLVED (Alec → reactive render-gate; fix: `SuggestionsList.tsx` derives `showEmptyBudgetNotice = optimizationNotice && !isOptimizing && unspentPassivePoints <= 0` from a live `useBuildStore(selectUnspentPassivePoints)` read so the notice auto-hides the moment points are re-allocated and the generic empty-state copy returns; regression test added).** _Original finding:_ After the guard sets `optimizationNotice`, if the user follows the notice's own advice and edits the **same** build (raises `characterLevel` or allocates a node so unspent > 0), the notice is never cleared: `App.tsx:81` short-circuits the build-switch subscriber on unchanged `activeBuild.id`, so `clearSuggestions()` — the only clearer of `optimizationNotice` — does not run until the next `startOptimization`. The panel keeps showing "No unspent passive points available. Allocate additional points…" while the build now visibly has points (self-corrects on the next Optimize click). Same staleness pattern as the pre-existing stale-`suggestions` behavior (subscriber clears only on id change), but the notice is a definitive false claim rather than merely old data. Source: Edge Case Hunter. Location: set `useOptimizationStream.ts:54-56`, not-cleared `App.tsx:81-85`, rendered `SuggestionsList.tsx:486`.
- [x] [Review][Patch] Empty-budget message copy mismatched for negative (over-allocated) and level 1-2 budgets — **RESOLVED (Alec → differentiate copy; fix: `useOptimizationStream.ts` keeps the verbatim AC3 `EMPTY_BUDGET_MESSAGE` for the all-spent zero case [AC3 still MET], adds `OVER_BUDGET_MESSAGE` for negative/over-allocated and `LOW_LEVEL_BUDGET_MESSAGE` for level 1-2 via an `emptyBudgetMessage(available, unspent)` selector; branch tests added).** _Original finding:_ The guard fires on `selectUnspentPassivePoints(...) <= 0`, so the verbatim AC3 message ("…Allocate additional points…") also shows for (a) over-allocated unenforced builds (negative unspent — "allocate additional points" is the opposite of correct) and (b) level 1-2 builds (0 budget — correct advice is "level up"). AC3 mandates the verbatim string for the **zero** case (satisfied) but is silent on these sub-cases. No crash; the guard correctly avoids the paid Claude call — only the copy is off for two uncommon states. Source: Acceptance Auditor + Edge Case Hunter. Location: guard `useOptimizationStream.ts:54`, message `useOptimizationStream.ts:38-39`.

...
## Decisions & risks (RESOLVED with Alec — 2026-07-02)

All four decisions are **RESOLVED** (Alec, 2026-07-02) — recommended defaults confirmed. dev-story should treat them as settled and NOT re-open them.

- **D1 — RESOLVED (Alec, recommended): do NOT add a `position` field to the Rust `build_snapshot::AffixEntry`.** Equipped-affix position is already encoded structurally by which `GearSlotSnapshot` vec the affix lives in (`prefixes`/`suffixes`), and all consumers read it that way. AC1's "the Rust affix type gains a position field" is satisfied by the existing `GearAffixData.affix_class` enum (corpus) + the vec split (equipped). Adding a redundant field creates two sources of truth that can desync. **Recommendation: TS `AffixEntryV2.position` only; Rust unchanged on the snapshot path.** (Flag for code-review as an intentional deviation from the ADR's literal "Rust affix type" wording — the vec split predates and supersedes it, from 4.0.)
- **D2 — RESOLVED (Alec, recommended): keep the corpus/item-DB field name `type`; name the new build field `position`; map at the boundary.** Avoids a corpus rename + regen and avoids the silent-all-prefix trap (a deserializer keyed on `position` over `type`-named data). No data or `generate_item_db.py` change.
- **D3 — RESOLVED (Alec): `'implicit'` → `'prefix'`.** The item-DB `AffixEntry.type` has a third value `'implicit'`; LE treats implicits as non-suffix and the prefix/suffix count rules govern explicit affixes. Map implicit to prefix.
- **D4 — ACCEPTED (Alec: "it's fine"): proceed from current HEAD.** No reviewed "Story 4.0 complete" baseline commit is required before 4.1 — provenance acknowledged (4.0's data half is AutoSave-only in `7730e4b`/`9137ad6`; working tree verified clean and 4.0's work present on disk). The intended `1651→1217` statKey index-grouping reduction and the AutoSaved `__pycache__/*.pyc` are noted, non-blocking.

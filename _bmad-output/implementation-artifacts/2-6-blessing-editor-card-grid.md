...
- [ ] [Review][Decision] Single-select group modeled as N independent `aria-pressed` toggle buttons, not a radiogroup — Each card's option set (None + blessings) is mutually exclusive, but rendered as a list of `<button aria-pressed>` toggles inside a plain `<div>` (no `role="radiogroup"`/`role="radio"`/`aria-checked`, no group label, no roving-tabindex arrow nav). To assistive tech this reads as independent toggles, not "choose exactly one." AC6 explicitly sanctions `aria-pressed` (and `vitest-axe` passes — multiple `aria-pressed="false"` + one `true` is valid ARIA, so axe is blind to the semantic mismatch), so this is **not an AC violation** — it's a deliberate a11y-pattern choice. Choosing the radiogroup pattern would also resolve the "None disambiguation" patch below. [BlessingsPanel.tsx:36-38]
- [ ] [Review][Patch] Duplicate, context-free accessible names for the "None" rows [lebo/src/features/blessings/BlessingsPanel.tsx:88] — Every card renders a button whose entire accessible name is "None"; with 12 timelines on screen a screen-reader user hears "None button" 12× with no programmatic tie to the owning timeline (the visible timeline-name `<p>` is not associated). Unambiguous additive a11y fix: give the None row (and/or each row) an `aria-label` that includes `group.timelineName`, or wrap each card's options in a labelled group. No data-wiring/token/dep change. (Superseded if the radiogroup decision above is adopted with a group `aria-label`.)
- [ ] [Review][Patch] Test gap — no assertion that a card renders ALL of its timeline's entries [lebo/src/features/blessings/BlessingsPanel.test.tsx:96-106] — The mock supplies only 2 entries (blood-frost-death) / 1 (age-of-winter); real data is 4 per timeline. The rewritten tests assert the mocked names by name but never assert completeness/count, so a regression that dropped entries past index 1 would still pass (the old dropdown enumerated options). Add a count/enumeration assertion (e.g. each card has `entries.length + 1` buttons incl. None).

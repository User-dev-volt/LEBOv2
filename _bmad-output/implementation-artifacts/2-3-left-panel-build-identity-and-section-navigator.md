...
- [x] [Review][Patch] Harden fill-count counters against missing `itemName`/`skillName` [lebo/src/shared/utils/buildSectionStatus.ts:19,23] — FIXED: `(g.itemName ?? '').trim()` / `(s.skillName ?? '').trim()`. — `g.itemName.trim()` / `s.skillName.trim()` throw `TypeError` if a persisted **v2** build has an element missing the key (the v2 load path in `buildPersistence.ts` does not coerce per-element strings; only v1→v2 migration does). `getSectionStatus(activeBuild)` runs unconditionally at the top of `LeftPanel` render, so the throw blanks the **entire** left panel. Pre-existing hazard relocated by this diff — cheap to fix here: `(g.itemName ?? '').trim()` / `(s.skillName ?? '').trim()`. (High, blind+edge)
- [x] [Review][Patch] Close `LeftPanel.test.tsx` coverage gaps [lebo/src/features/layout/LeftPanel.test.tsx] — FIXED: (a) added an empty/blank-name slot exclusion test (gear `1/11`, skills `1/5`, no checkmark); (b) typed the `gear()`/`skill()`/new `idol()` helpers to the real `GearItemV2`/`ActiveSkill`/`PlacedIdol` shapes; (c) added `done`-checkmark tests for Skill Trees (`pts>=1`), Idols (`>=1`), Blessings (`>=1`, asserting the null slot is ignored); (d) added a `ClassGlyph` describe block — all 5 class motifs render distinct svgs (`aria-hidden`), unknown id → circle fallback, fallback shares no class motif. Suite: 11 → 18 tests, all green. (Medium, edge)

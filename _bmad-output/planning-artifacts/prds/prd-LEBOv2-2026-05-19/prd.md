...
FR-F6: The Weaver Tree canvas renders a distinctive background using a separate `TilingSprite` backed by a 256×256 WebP void/crystalline texture (`bg_weaver_tile.webp`) with a purple-tinted dark aesthetic, visually distinct from the stone-textured passive and skill tree backgrounds. Same `TilingSprite` approach as FR-F4; sourced via icon pipeline extraction or synthetic fallback.
